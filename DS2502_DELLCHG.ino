...
const int pin_45w = 4;  // if pin down set psu data -> 45w otherwise set 130w
const int pin_65w = 6;  // if pin down set psu data -> 65w otherwise set 130w
const int pin_90w = 9;  // if pin down set psu data -> 90w otherwise set 130w
...
void setup()
{
    pinMode(pin_45w, INPUT_PULLUP);
    pinMode(pin_65w, INPUT_PULLUP);
    pinMode(pin_90w, INPUT_PULLUP);
    //Serial.begin(115200);
    ...
    if (digitalRead(pin_45w) == LOW) {
        dellCH.writeMemory((uint8_t *) charger45W, chargerStrlen);
    } else if (digitalRead(pin_65w) == LOW) {
        dellCH.writeMemory((uint8_t *) charger65W, chargerStrlen);
    } else if (digitalRead(pin_90w) == LOW) {
        dellCH.writeMemory((uint8_t *) charger90W, chargerStrlen);
    } else {
